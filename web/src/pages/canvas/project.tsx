...
function InfiniteCanvasPage() {
    ...
    const uploadMaterialFileNode = useCallback(async (file: File, kind: MaterialFileKind): Promise<CanvasNodeData> => {
        if (kind === "audio") {
            const audio = await uploadMediaFile(file, "audio");
            const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
            return {
    ...
    const handleUploadRequest = useCallback((mode: MaterialUploadMode, nodeId?: string, position?: Position) => {
        const input = imageInputRef.current;
        if (!input) return;
        input.multiple = mode === "multiple";
        uploadTargetRef.current = { mode, nodeId, position };
        input.click();
    }, []);
    ...
    const handleSetGroupSelectionChange = useCallback((nodeId: string, nodeIds: string[]) => {
        setNodes((current) =>
            current.map((node) => {
                if (node.id !== nodeId || node.type !== CanvasNodeType.BatchInfo) return node;
                const state = readBatchIntakeState(node.metadata);
                if (state.batch_type !== "set" || (state.status !== "draft" && state.status !== "failed")) return node;
                return {
                    ...node,
                    metadata: {
                        ...node.metadata,
                        content: undefined,
                        batchIntake: resetBatchDeclarationState(state, setGroupSelectionPatch(nodeIds)),
                    },
                };
            }),
        );
    ...
    const handleImageInputChange = useCallback(
        async (event: ReactChangeEvent<HTMLInputElement>) => {
            ...
            if ((mode === "single" || target?.nodeId) && files.length > 1) {
                message.warning("此入口一次只能选择 1 个素材，请重新选择。");
                return;
            }
            ...
            if (target?.nodeId) {
                ...
                if (!kind) {
                    message.warning("该文件不是支持的图片、视频或音频素材。");
                    return;
                }
                if (kind === "audio") {
                    const audio = await uploadMediaFile(file, "audio");
            ...
            const anchor = target?.position || screenToCanvas((containerRef.current?.getBoundingClientRect().left || 0) + size.width / 2, (containerRef.current?.getBoundingClientRect().top || 0) + size.height / 2);
            ...
            const result = await runMaterialUploadBatch({
                ...
                uploadFile: async (file, kind) => {
                    const node = await uploadMaterialFileNode(file, kind);
                    uploadedNodes.push(node);
                    return node;
                },
                commitNode: (update) => setNodes((current) => update(current)),
            });
            const focus = materialUploadFocus(uploadedNodes.slice(0, result.uploadedCount));
            if (focus.selectedNodeId) {
                setSelectedNodeIds(new Set([focus.selectedNodeId]));
                setSelectedConnectionId(null);
                if (focus.dialogNodeId) setDialogNodeId(focus.dialogNodeId);
            }
            if (result.status === "rejected") message.warning(result.message);
            if (result.status === "failed") message.error(result.message);
        },
        [message, screenToCanvas, size.height, size.width, uploadMaterialFileNode],
    );
    ...
    const handleDrop = useCallback(
        ...
        [message, screenToCanvas, uploadMaterialFileNode],
    );
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                <CanvasTopBar
                    ...
                    onImportImage={() => handleUploadRequest("single")}
                    onUndo={undoCanvas}
                    onRedo={redoCanvas}
                            ...
                            renderNodeContent={(contentNode) =>
                                contentNode.type === CanvasNodeType.BatchInfo ? (
                                    <CanvasBatchInfoNode
                                        ...
                                        connectedImages={batchConnectedImages(contentNode.id, nodes, connections)}
                                        setGroupFileNames={batchRoleImageFileNames(contentNode, nodes, "setGroupImageNodeIds")}
                                        componentWhiteBgFileNames={batchRoleImageFileNames(contentNode, nodes, "componentWhiteBgImageNodeIds")}
                                        ...
                                        onSetGroupSelectionChange={handleSetGroupSelectionChange}
                ...
                <CanvasNodeHoverToolbar
                    ...
                    onGenerateImage={generateImageFromTextNode}
                    onUpload={(node) => handleUploadRequest("single", node.id)}
                ...
                <CanvasToolbar
                    ...
                    onUndo={undoCanvas}
                    onRedo={redoCanvas}
                    onUpload={() => handleUploadRequest("multiple")}
                ...
                <CanvasWorkflowCostCard
                    open={Boolean(workflowDemo.pendingNodeId)}
                ...
                <CanvasDeleteProjectsDialog
                    projectIds={deleteCurrentOpen ? [projectId] : []}
                    onDeleted={() => navigate("/canvas")}
                    onClosed={() => setDeleteCurrentOpen(false)}
                />
                    ...
                    <Dropdown
                        ...
                        menu={{
                            items: [
                                ...
                                { key: "delete", danger: true, icon: <Trash2 className="size-4" />, label: "删除当前项目", onClick: onDeleteProject },
                                { type: "divider" },
...
function batchConnectedImages(batchInfoNodeId: string, nodes: CanvasNodeData[], connections: CanvasConnection[]) {
    const nodesById = new Map(nodes.map((node) => [node.id, node]));
    return connectedBatchOriginalImageIds(batchInfoNodeId, nodes, connections).map((id) => ({
        id,
        name: nodesById.get(id)?.metadata?.sourceFile?.name || nodesById.get(id)?.title || id,
    }));
}

function batchRoleImageFileNames(card: CanvasNodeData, nodes: CanvasNodeData[], field: "setGroupImageNodeIds" | "componentWhiteBgImageNodeIds") {
    const nodesById = new Map(nodes.map((node) => [node.id, node]));
