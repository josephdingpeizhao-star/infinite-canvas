...
import type { CanvasNodeData, CanvasWorkflowDemoStatus } from "@/types/canvas";

export function CanvasWorkflowNode({ node, connectedImageCount, onStart, onToggleDetails }: { node: CanvasNodeData; connectedImageCount: number; onStart: (nodeId: string) => void; onToggleDetails: (nodeId: string) => void }) {
    ...
    return (
        <div className="flex h-full w-full cursor-move flex-col gap-3 p-4" style={{ color: theme.node.text }}>
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-[11px]">
                <SummaryCell label="已连接" value={`${connectedImageCount} 张`} />
...
export function CanvasWorkflowNodePanel({ onClose }: { onClose: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const fields = [
        ["品类", "保温杯演示"],
        ["高度", "24 厘米"],
    ...
    return (
        <div className="rounded-2xl border p-4 shadow-xl backdrop-blur" style={{ borderColor: theme.toolbar.border, background: theme.toolbar.panel, color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
                <div>
                    <div className="text-sm font-semibold">演示信息</div>
            ...
            <div className="mt-3 text-[11px]" style={{ color: theme.node.muted }}>
                共 {WORKFLOW_DEMO_TOTAL} 张 · 演示模式不产生任何费用
            </div>
        </div>
    );
}
