...
function machine(expectedConfigIds = CONFIG_IDS): CanvasNodeData {
    return {
        id: "machine",
        ...
        width: 420,
        height: 300,
        metadata: { workflowProduction: { status: "completed", producedCount: expectedConfigIds.length, totalCount: expectedConfigIds.length, expectedConfigIds } },
    };
}
...
describe("canvas receiving box", () => {
    ...
    test("shows confirm only after every batch-defined slot is present", () => {
        const expectedConfigIds = CONFIG_IDS.slice(0, 3).concat(CONFIG_IDS.slice(6, 8));
        const box = createReceivingBox(machine(expectedConfigIds), "cup");
        const partial = expectedConfigIds.slice(0, 4).map((configId) => ({ ...image(configId), metadata: { ...image(configId).metadata, groupId: box.id } }));
        const full = [...partial, { ...image(expectedConfigIds[4]!), metadata: { ...image(expectedConfigIds[4]!).metadata, groupId: box.id } }];
        expect(receivingBoxView(box, [box, ...partial])).toMatchObject({ count: 4, totalCount: 5, canConfirm: false });
        expect(receivingBoxView(box, [box, ...full])).toMatchObject({ count: 5, totalCount: 5, canConfirm: true });
    });

    test("builds the exact batch-sized closeout payload with source and SHA", () => {
        const box = createReceivingBox(machine(), "cup");
    ...
    test("submits closeout once through the token-protected JSON endpoint", async () => {
        ...
        const result = await submitAcceptanceCloseout("cup", " token ", payload, async (_input, init) => {
            captured = init;
            return new Response(JSON.stringify({ ok: true, batchId: "cup", status: "closed", totalCount: 14, expectedConfigIds: CONFIG_IDS, closedAt: "2026-07-24T12:00:00" }), { status: 200 });
        });
    ...
    test("restores an already-closed status without returning final selections", async () => {
        const result = await fetchAcceptanceStatus("cup", "token", async () => new Response(JSON.stringify({ ok: true, batchId: "cup", status: "closed", totalCount: 14, expectedConfigIds: CONFIG_IDS, closedAt: "2026-07-24T12:00:00" }), { status: 200 }));
        expect(result.status).toBe("closed");
        expect("selections" in result).toBe(false);
    });

    test("fails closed instead of restoring a frontend total when count facts are missing", async () => {
        expect(() => createReceivingBox({ ...machine(), metadata: {} }, "cup")).toThrow(WORKFLOW_COUNT_DATA_MISSING_MESSAGE);
        await expect(fetchAcceptanceStatus("cup", "token", async () => new Response(JSON.stringify({ ok: true, batchId: "cup", status: "open" }), { status: 200 }))).rejects.toThrow(WORKFLOW_COUNT_DATA_MISSING_MESSAGE);
    });
});
